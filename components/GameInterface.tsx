...
const GameInterface: React.FC<Props> = ({ character, updateCharacter, onPvP, onGameOver }) => {
  ...
  const initialWorld: WorldState = loggedInUser?.worldState || {
      ...
      arcProgress: 0,
      currentLocation: character.origin === 'Maldição' ? "Floresta das Sombras" : "Portão da Escola de Jujutsu",
      canonDivergence: 0,
      notableChanges: [],
  ...
  useEffect(() => {
    if (history.length === 0) {
        const welcomeAction = character.origin === 'Maldição' ? "Despertar como maldição" : "Chegar na Escola de Jujutsu";
        handleAction(welcomeAction);
    }
  }, []);
  ...
  const handleAction = async (action: string) => {
    ...
    try {
      ...
      setHistory(prev => [...prev, { role: 'narrator', content: "A fenda na realidade causou uma instabilidade energética." }]);
    } finally {
      setIsThinking(false);
      setInput('');
    }
  ...
  return (
    <div className={`flex flex-col h-screen w-full max-w-6xl mx-auto overflow-hidden relative ${shake ? 'shake' : ''}`}>
          ...
          <div className="flex justify-between items-start max-w-4xl mx-auto w-full gap-4">
              <div className="glass-panel p-3 rounded-2xl border border-white/10 w-48 md:w-64 pointer-events-auto shadow-2xl">
                  <div className="flex items-center gap-3 mb-2">
                      <img src={character.profileImageUrl} className="w-10 h-10 rounded-lg border border-purple-500 object-cover bg-black" alt="Profile" />
                      <div className="overflow-hidden">
                          <div className="text-[10px] font-bungee text-white leading-none mb-0.5 truncate">{character.name}</div>
                          <div className="text-[8px] font-mono text-purple-400 truncate">{character.grade}</div>
                      </div>
                  </div>
              ...
              <div className="flex flex-col items-end gap-2 pointer-events-auto">
                  <div className="glass-panel px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 shadow-lg bg-black/60">
                  ...
                  <div className="flex gap-2">
                      <button onClick={() => setShowBonds(true)} className="p-3 glass-panel rounded-xl border border-white/10 text-lg hover:bg-white/5 active:scale-90 transition-all shadow-lg bg-black/60" title="Vínculos">👥</button>
                      <button onClick={() => setShowHub(true)} className="p-3 glass-panel rounded-xl border border-white/10 text-lg hover:bg-white/5 active:scale-90 transition-all shadow-lg bg-black/60" title="Menu">⚙️</button>
                  </div>
              </div>
          </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pt-44 pb-48 px-4 md:px-12 space-y-10 no-scrollbar" ref={scrollRef}>
          {history.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'player' ? 'justify-end' : 'justify-start'} animate-message ${msg.kokusen ? 'kokusen-active' : ''}`}>
                  <div className={`max-w-2xl w-full p-6 md:p-8 rounded-[2rem] border shadow-2xl ${
                      msg.role === 'player' 
                      ? 'bg-purple-900/15 border-purple-500/30 text-purple-100 font-marker text-xl md:text-2xl text-right' 
                      : 'bg-black/80 border-white/5 text-gray-200 backdrop-blur-md'
                  }`}>
                      {msg.imageUrl && (
                          <div className="relative group mb-6 overflow-hidden rounded-2xl border border-white/10">
                              <img src={msg.imageUrl} className="w-full h-auto cursor-zoom-in group-hover:scale-105 transition-transform duration-500" onClick={() => setExpandedImage(msg.imageUrl!)} alt="Scene" />
                          </div>
                      )}
                      
                      {msg.npcIntervention && (
                          <div className="mb-6 p-4 bg-purple-600/20 border border-purple-500/50 rounded-xl flex items-center gap-4 animate-pulse">
                              <div className="text-xl">⚡</div>
                              <div className="flex flex-col">
                                  <span className="text-[9px] font-bungee text-purple-400 uppercase">Resgate</span>
                                  <span className="text-sm font-marker text-white">{msg.npcIntervention} interviu!</span>
                              </div>
                          </div>
                      )}

                      <div className="text-lg md:text-xl leading-relaxed whitespace-pre-wrap font-inter font-medium tracking-tight">
                          {msg.content}
                      </div>

                      {msg.sources && msg.sources.length > 0 && (
                          <div className="mt-6 border-t border-white/10 pt-4 flex flex-wrap gap-2">
                              {msg.sources.map((source, sIdx) => (
                                  <a key={sIdx} href={source.uri} target="_blank" rel="noopener noreferrer" className="text-[10px] text-purple-400 bg-purple-500/5 border border-purple-500/20 px-2 py-1 rounded-md hover:bg-purple-500/10 transition-colors">
                                      {source.title}
                                  </a>
                              ))}
                          </div>
          ...
          {isThinking && (
              <div className="flex justify-center py-6">
                  <div className="flex items-center gap-3 glass-panel px-6 py-3 rounded-full border border-purple-500/30 bg-black/80">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-40 p-4 pb-8 md:p-8 bg-gradient-to-t from-black via-black/90 to-transparent">
          <div className="max-w-4xl mx-auto space-y-4">
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                  {suggestions.map((s, idx) => (
                      <button key={idx} onClick={() => handleAction(s)} disabled={isThinking} className="px-5 py-2.5 glass-panel border border-white/5 rounded-full text-[9px] font-bungee text-white/40 hover:text-white hover:border-purple-500 transition-all uppercase whitespace-nowrap active:scale-95 bg-black/40">
                          {s}
                      </button>
                  ))}
              </div>
              <form onSubmit={e => { e.preventDefault(); if(input.trim()) handleAction(input); }} className="flex gap-3">
                  <input className="flex-1 glass-panel border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-purple-500 outline-none transition-all font-inter text-lg shadow-2xl bg-black/60" placeholder="O que você faz?..." value={input} onChange={e => setInput(e.target.value)} disabled={isThinking} />
                  <button disabled={isThinking || !input.trim()} className="px-8 bg-white text-black font-bungee rounded-2xl hover:bg-purple-600 hover:text-white transition-all active:scale-95 shadow-xl text-sm">ENVIAR</button>
              </form>
          </div>
      </div>
      ...
      {showBonds && (
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4">
              <div className="w-full max-w-2xl glass-panel rounded-[2rem] border border-white/10 overflow-hidden flex flex-col max-h-[80vh] shadow-2xl">
                  <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/40">
                      <h3 className="font-bungee text-white text-xl uppercase">Vínculos de Alma</h3>
                      <button onClick={() => setShowBonds(false)} className="text-3xl text-white/40 hover:text-white transition-all">×</button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                      {Object.values(worldState.npcRelationships).length === 0 ? (
                          <div className="text-center opacity-20 py-20 font-bungee">Sua jornada apenas começou...</div>
                      ) : (
                          (Object.values(worldState.npcRelationships) as NPCRelationship[]).map((npc, i) => (
                              <div key={i} className="p-6 rounded-2xl border bg-white/5 border-white/5 hover:border-purple-500/30 transition-all group">
                                  <div className="flex justify-between items-center mb-3">
                                      <span className="font-marker text-2xl text-white group-hover:text-purple-400 transition-colors">{npc.name}</span>
                                      <span className="text-[10px] font-bungee text-purple-400">{npc.affinity}% Vínculo</span>
                                  </div>
                                  <div className="h-1.5 bg-black/60 rounded-full overflow-hidden p-0.5">
                                      <div className="h-full bg-gradient-to-r from-purple-600 to-blue-400 rounded-full" style={{width: `${npc.affinity}%`}}></div>
                                  </div>
                              </div>
                          ))
                      )}
